    ...
    TreeNode *buildTree(vector<int> &preorder, vector<int> &inorder)
    {
        //
		 ...
		 //       //in_pre = find(preorder.begin(),preorder.end(),*(--in)); //find the middle of the preorder
			...
			//	  for(vector<int>::iterator it1= inorder.begin();  it1 != in;  it1++,it2++)  ;  //it2 marks the middle of preorder
		 //        vector<int> inLeft(inorder.begin(),in); //in has becomes the last element of the left after --
		 //        vector<int> inRight(++in,inorder.end());
		 ...
		 //return node;		       
        //      
		return 0 == preorder.size()?NULL:createTreeTemp(preorder.begin(),preorder.end(),inorder.begin(),inorder.end());

    }
	TreeNode* createTreeTemp(vector<int>::iterator preL,vector<int>::iterator preR, vector<int>::iterator inL, vector<int>::iterator inR)
	{
		   if(preL >= preR) 
			       return NULL;
		   int value = *preL;
